...
MODEL_LIST = ['gemini-2.0-flash', 'gemini-2.0-pro']
...
st.sidebar.header("Enter your Google API_Key API Key")
...
model_option = st.sidebar.selectbox("Choose Google model", MODEL_LIST, index=0)
...
def process_exploratory(question: str, llm, data: pd.DataFrame) -> dict:
    ...
    if tool_calls:
        ...
        if tool_name == "explain_data":
            ...
            if artifacts and isinstance(artifacts, dict):
                st.write("Debugging Artifacts:", artifacts)  # Check what's inside
